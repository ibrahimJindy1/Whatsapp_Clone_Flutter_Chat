...
import 'package:chat/utils/ChatRequestProvider.dart';
...
class ChatScreenState extends State<ChatScreen> with WidgetsBindingObserver {
  ...
  void sendMessage({FilePickerResult? result, String? stickerPath}) async {
    ...
    if (await chatRequestService.isRequestsUserExist(widget.user!.uid!)) {
  ...
  void sendNormalMessages(ChatMessageModel data, {FilePickerResult? result}) async {
    if (isFirstMsg) {
      ...
      data.uid = widget.user!.uid;
      data.addedOn = Timestamp.now();
      data.lastMessageTime = DateTime.now().millisecondsSinceEpoch;

      chatMessageService.getContactsDocument(of: getStringAsync(userId), forContact: widget.user!.uid).set(data.toJson()).then((value) {
        //
      }).catchError((e) {
        log(e);
      });
    ...
    await chatMessageService.addMessage(data).then((value) async {
      ...
      await chatMessageService.addMessageToDb(senderDoc: value, data: data, sender: sender, user: widget.user, image: result != null ? File(result.files.single.path!) : null, isRequest: false).then((value) {
        //
      });
    });
  ...
  void sendChatRequest(ChatMessageModel data, {FilePickerResult? result}) async {
    if (!widget.user!.oneSignalPlayerId.isEmptyOrNull) {
      notificationService.sendPushNotifications(getStringAsync(userDisplayName), messageCont.text, receiverPlayerId: widget.user!.oneSignalPlayerId).catchError(log);
    }
    messageCont.clear();
    ...
    if (await chatRequestService.isRequestUserExist(sender.uid!, widget.user!.uid.validate())) {
      log("hello");
      chatMessageService.addMessage(data).then((value) async {
        ...
        await chatMessageService.addMessageToDb(senderDoc: value, data: data, sender: sender, user: widget.user, image: result != null ? File(result.files.single.path!) : null, isRequest: true).then((value) {
          //
        });
      });
    } else {
      chatRequestService.addChatWithCustomId(sender.uid!, chatReq.toJson(), widget.user!.uid.validate()).then((value) {
      ...
      child: SafeArea(
        child: Scaffold(
          appBar: PreferredSize(
            ...
            child: ChatAppBarWidget(receiverUser: widget.user!),
          ),
          body: Container(
            child: FutureBuilder<bool>(
                future: chatRequestService.isRequestsUserExist(widget.user!.uid!),
                  ...
                  return Stack(
                    ...
                    children: [
                      ...
                      PaginateFirestore(
                        ...
                        query: chatMessageService.chatMessagesWithPagination(currentUserId: getStringAsync(userId), receiverUserId: widget.user!.uid!),
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              8.height,
              Text('if you accept the invite, ${widget.user!.name.validate()} can message you.', style: primaryTextStyle(color: Colors.white70)),
              16.height,
              Row(
                ...
                children: [
                  AppButton(
                    ...
                    onTap: () {
                      chatRequestService.removeDocument(widget.user!.uid.validate()).then((value) {
                        chatMessageService.deleteChat(senderId: getStringAsync(userId), receiverId: widget.user!.uid.validate()).then((value) {
                          finish(context);
                          finish(context);
                        }).catchError((e) {
                          log(e.toString());
                  ...
                  AppButton(
                    ...
                    onTap: () async {
                      ...
                      chatRequestService.updateDocument({"requestStatus": RequestStatus.Accepted.index}, widget.user!.uid).then((value) => null).catchError(
                            (e) {
                              log(e.toString());
                            },
                          );
